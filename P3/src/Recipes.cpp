...
string Recipes::get_vegeterian_state()
{
	return is_it_veg;
}
int Recipes::get_minutes_to_ready()
{
...
string Recipes::get_tags()
{
	string info;
	int tags_size = (int)tags.size();
	for (int tags_curser = 0; tags_curser < tags_size; tags_curser++)
	{
...
float Recipes::was_he_rated(string name)
{
	...
	for (rate_counter = 0; rate_counter < detail_rated.size(); rate_counter++)
	{
		...
		{
			return detail_rated[rate_counter].rate;
		}
	}
	return 0;
}
string Recipes::get_image_address()
{
